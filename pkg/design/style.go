package design

import "github.com/charmbracelet/lipgloss"

const (
	COLOR_RED    = lipgloss.Color("#EB2D3A")
	COLOR_BLUE   = lipgloss.Color("#0069D9")
	COLOR_PINK   = lipgloss.Color("#D900C7")
	COLOR_GREY   = lipgloss.Color("#555555")
	COLOR_WHITE  = lipgloss.Color("#D9DCCF")
	COLOR_BLACK  = lipgloss.Color("#000000")
	COLOR_GREEN  = lipgloss.Color("#3AF191")
	COLOR_PURPEL = lipgloss.Color("#7F00D9")
	COLOR_ORANGE = lipgloss.Color("#D98D00")
	COLOR_YELLOW = lipgloss.Color("#FFDF00")
)

type Style struct {
}

func MakeStyle() {

}

func (s Style) StatusCode(v int) {

}
